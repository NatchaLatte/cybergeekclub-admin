...
export default function EditNews(): JSX.Element {
  ...
  const getData = async () => {
    try {
      const response = await axios.post(`/api/news/${params.id}`, {
        id: params.id,
      });
      if (response.data) {
        setRawData({...response.data.data, email: data.user.email});
      }
    } catch (error: unknown) {
      console.error(error);
    }
    ...
    if (!display) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            setDisplay(true);
            await axios.post(`/api/news/${params.id}/update`, rawData, {
              headers: {
  ...
  return status === "authenticated" ? (
    <main className="flex flex-row h-screen justify-center items-center gap-5">
      <div className="flex flex-col">
        <h1 className="text-3xl my-5 text-center">Edit News</h1>
        <form
