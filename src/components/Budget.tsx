...
export default function Budget(): JSX.Element {
  ...
  const handleSearch = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const dataTableSearchFilter = dataTable.filter((value: any) => {
      const category = value.category ? value.category.toLowerCase() : "";
  ...
  const handleDelete = async (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          setDisplay(true);
          await axios.post("/api/budget/delete", {
            id: id,
          });
          getData();
          await Toast.fire({
    ...
    <main>
      <div className="flex flex-col md:flex-row gap-3 m-5">
        <input
          ...
          value={search}
          placeholder="Category or Particulars"
              ...
              dataTableSearch.map((data: any, index) => {
                return (
                  <tr key={data.id}>
                    ...
                    <td className="text-center">{data.money}</td>
                    <td className="text-center">
                      {data.time_series
                        ? moment.tz(data.time_series, "Asia/Bangkok").locale("th").format("L")
                        : "Not specified"}
                    </td>
                  </tr>
                );
